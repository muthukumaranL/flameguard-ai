...
The selected final model (Tuned final) reaches mAP@0.5 = 0.492, mAP@0.5:0.95 = 0.242, precision = 0.650 and recall = 0.426 on the held-out test split, which was evaluated exactly once. Per class, AP@0.5 is 0.516 for Fire and 0.467 for Smoke. Measured inference speed on the project laptop is 29.1 ms per image (34.4 FPS) on GPU and 246 ms (4.1 FPS) on CPU. The model runs inside a Streamlit application offering image upload, video processing and live browser webcam detection, with an OpenCV desktop fallback.
...
This project ran on a single 4 GB laptop GPU on which mixed precision is automatically disabled, and the experiment programme was sized to that constraint. Two things were shortened or left unfinished, and we state them here rather than leaving a reader to discover the gap:
...
|---|---|---|---|---|---|---|
| 0.200 | 0.537 | 0.454 | 0.492 | 995 | 647 | 819 |
| 0.300 | 0.536 | 0.454 | 0.491 | 543 | 745 | 722 |
| 0.400 | 0.641 | 0.395 | 0.489 | 318 | 843 | 627 |
| 0.500 | 0.748 | 0.337 | 0.465 | 171 | 937 | 533 |
| 0.600 | 0.846 | 0.256 | 0.393 | 73 | 1056 | 415 |

F1 is essentially flat across the low end of the range and reaches its numerical maximum at 0.20 (F1 0.492), but we did not simply take the argmax. At 0.20 and at 0.30 the recall is identical (0.454) and the F1 differs by less than 0.001, yet the lower threshold roughly doubles the false-positive count for no gain in recall whatsoever. The selection rule therefore keeps every threshold within 0.005 F1 of the best, discards any that would sacrifice recall, and among the survivors takes the one with the fewest false positives - which is 0.30 (precision 0.536, recall 0.454, F1 0.491). That is the application's default. The shape of the curve is the real answer to RQ3: raising the threshold buys precision cheaply at first and then starts destroying recall, and for a safety detector the right place to sit is at the low end of the flat region of the F1 curve - just not so low that false alarms pile up for nothing - because a false negative (a fire nobody is told about) is a categorically worse outcome than a false positive (an operator glances at a camera and dismisses it). The application exposes the threshold as a slider so this trade-off can be made explicitly rather than silently.
...
|---|---|---|---|---|
| GPU (GTX 1650 Ti) | 29.1 | 28.8 | 39.3 | 34.4 |
| CPU | 245.7 | 160.6 | 685.8 | 4.1 |

Speed was measured by timing repeated single-image predictions end to end - including pre-processing and NMS - rather than by reading a theoretical FLOP count. At 34.4 FPS (29.1 ms per image) on the GPU, the model is fast enough to drive a live camera feed. On CPU it runs at 4.1 FPS, which is slower but still usable for image and video analysis - and that matters, because the application has to run on whatever machine is in the room.
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| Tuned final | 5.960 | 0.537 | 0.454 | 0.492 | 0.479 | 0.218 | 0.494 | 0.413 | 7.713 | 129.649 | 0.443 |
| YOLO11n | 5.220 | 0.525 | 0.410 | 0.461 | 0.435 | 0.200 | 0.450 | 0.371 | 7.924 | 126.196 | 0.368 |
| YOLOv8n (baseline) | 5.960 | 0.569 | 0.446 | 0.500 | 0.491 | 0.225 | 0.482 | 0.409 | 8.385 | 119.260 | 0.293 |
...
Answering RQ2 fairly. The baseline ran for 40 epochs, while the other architectures ran for far fewer - not because they are worse, but because the GPU budget was fixed and they cost more per epoch. Comparing final numbers would therefore compare training length, not architecture. Because validation metrics are recorded after every epoch, we can instead compare each rival against the baseline AT THE EPOCH IT REACHED. YOLO11n at epoch 12: mAP@0.5 = 0.435, mAP@0.5:0.95 = 0.200, recall = 0.410. YOLOv8n at the same epoch 12: mAP@0.5 = 0.314, mAP@0.5:0.95 = 0.136, recall = 0.311. So YOLO11n is ahead of the baseline architecture by 0.121 mAP@0.5 (+0.099 recall) at equal training length. **The YOLOv8s capacity experiment was NOT completed, and we are not going to pretend otherwise.** It is absent from the table above because no training run of it finished before the submission deadline; there is therefore no YOLOv8s row, no interpolated row, and no estimate standing in for one. What we do have is a direct measurement of why it failed, and that is worth more than a half-trained number. We probed the model's real cost on this GPU (scripts/vram_probe.py, evidence in outputs/training/vram_probe.json), running genuine training iterations at each batch size on a NVIDIA GeForce GTX 1650 Ti with 4.0 GB of VRAM in FP32 (AMP auto-disabled by Ultralytics on GTX 16xx): batch 2: 1.0 GB peak, 12.3 img/s (fits in vram); batch 4: 6.08 GB peak, 2.6 img/s (spilling to shared memory); batch 8: 7.94 GB peak, 2.9 img/s (spilling to shared memory). For reference, the YOLOv8n baseline at batch 16 peaks at 4.84 GB and sustains 16.8 img/s on the same card. On Windows, exceeding physical VRAM does not raise an out-of-memory error - PyTorch quietly pages into shared system memory across the PCIe bus, so the run appears healthy while throughput collapses by roughly a factor of five. Only batch 2 keeps YOLOv8s inside the card, and at batch 2 a single epoch takes about five minutes, which put a usable run outside the time we had left. The conclusion we can defend is narrow but real: on a 4 GB GPU in FP32, YOLOv8s at 640 pixels is not merely slower than YOLOv8n - it is effectively untrainable at any batch size large enough to be worth using. The architecture question is instead answered by YOLO11n, which runs at the same batch size and image size as the baseline and is therefore a cleaner controlled comparison anyway. The benchmark table above reports each model at its own best epoch, which is the operationally honest view - what you actually get for the compute you actually spent. The two views answer different questions and should be read together.

Model selection did not simply take the highest mAP. We scored candidates with a recall-weighted rule - 0.35 x mAP@0.5:0.95 + 0.25 x overall recall + 0.25 x smoke recall + 0.15 x a normalised speed score - because for a fire detector the cost of a miss is asymmetric, and because smoke is both the harder class and the earlier warning signal. The winner was Tuned final (e5_final), with validation recall 0.454 and smoke recall 0.413.
...
Every test image was compared against its ground truth with IoU-based, class-aware matching at a 0.3 confidence threshold. Of 533 test images, 136 were fully correct, 207 were correctly-empty backgrounds, 31 produced false positives only, 50 missed objects only, 89 found the object but placed the box poorly, and 20 contained a mixture. In total the model produced 371 true positives, 118 false positives, 306 false negatives and 101 localisation errors.

Missed detections break down by class as {'Smoke': 91, 'Fire': 215}. In raw counts the model misses more Fire instances, but that is largely because fire is the more frequent class; the fairer measure is per-class recall, where Smoke is harder - test recall is 0.458 for Fire against 0.395 for Smoke. In other words, the model finds a larger share of the fires it is shown than of the smoke, which is exactly what the EDA predicted about thin, low-texture, low-contrast plumes; fire simply contributes more absolute misses because there is more of it. False positives break down as {'Fire': 87, 'Smoke': 31}. Inspecting the galleries, the recurring false-positive triggers are the ones a human would also hesitate over for a moment: bright cloud banks and haze on the horizon read as smoke; sunset glow, warm interior lighting and orange reflective surfaces read as fire. The recurring false negatives are small distant flames, thin translucent smoke against a bright sky, and fires at night where the flame is the only lit object in an otherwise black frame.
...
To find out how much of the model's decision rests on colour alone, we fed it images that contain no fire, no smoke, no texture and no structure whatsoever: flat colour fields and random noise, at the operating threshold of 0.3. Anything detected in these is by construction a false positive. The result is uncomfortable and informative: a uniform orange rectangle is reported as Fire with 0.97 confidence, and 4 of the 4 flat-colour fields trigger a detection at all (flat_orange, flat_red, flat_grey, flat_blue). Random noise, by contrast, produces 0 detections. The model has therefore learned a strong colour prior - warm, saturated, low-texture regions read as fire - rather than a structural understanding of flame. That single fact explains the entire false-positive gallery (sunsets, orange lamps, warm reflections) far better than any per-image inspection does, and it is why hard-negative mining, not architecture search, is the top item in our future work. This probe is cheap, it is reproducible (scripts/error_analysis.py), and we would recommend it to anyone evaluating a colour-cued detector.
...
Full log in outputs/test_report.txt. pytest summary line: ======================== 65 passed, 1 skipped in 9.84s ========================
...
|---|---|---|---|---|---|---|---|
| sample_01 | 0114_jpg.rf.7d28b3cf35e22b7436518923eaefd163.jpg | fire-only (success) | 1 | 1 | Fire 0.76 | Correct | All 1 annotated object(s) detected with correct class and well-placed boxes. |
| sample_02 | 0124_jpg.rf.3c39dfe4c07ee58c12916e77e77833b8.jpg | fire-only (success) | 1 | 1 | Fire 0.40 | Correct | All 1 annotated object(s) detected with correct class and well-placed boxes. |
| sample_03 | ck0kcnqqgk6li0848vp3bn5sx_jpeg_jpg.rf.1ee72ec1f7c71a721859797bacc49fe5.jpg | smoke-only (success) | 1 | 1 | Smoke 0.64 | Correct | All 1 annotated object(s) detected with correct class and well-placed boxes. |
| sample_04 | ck0kdgpnj8gvt0701oaod540q_jpeg_jpg.rf.6616bd1a246cad1ae8208cb31d269521.jpg | smoke-only (success) | 1 | 1 | Smoke 0.74 | Correct | All 1 annotated object(s) detected with correct class and well-placed boxes. |
| sample_05 | 1026_jpg.rf.8edbee8d3d0404e86fc62b94d41c1cc9.jpg | fire + smoke (success) | 2 | 2 | Smoke 0.43; Fire 0.32 | Correct | All 2 annotated object(s) detected with correct class and well-placed boxes. |
| sample_06 | 1230_jpg.rf.40c9c200a7b8fef604b49ccb3f1b90dc.jpg | fire + smoke (success) | 2 | 2 | Fire 0.71; Smoke 0.45 | Correct | All 2 annotated object(s) detected with correct class and well-placed boxes. |
...
| sample_08 | 0184_jpg.rf.dc5b7914ec6572bba8226ab75394c3e0.jpg | difficult (localisation / false positive) | 2 | 3 | Fire 0.60; Fire 0.54; Fire 0.51 | Partially correct | Mixed outcome: 2 correct, 1 false positive(s), 0 missed. |
...
| sample_10 | 0137_jpg.rf.bd99ec327b337a137cbd37ee1e89b168.jpg | additional success | 1 | 1 | Fire 0.32 | Correct | All 1 annotated object(s) detected with correct class and well-placed boxes. |
